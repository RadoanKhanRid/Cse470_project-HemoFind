<x-app-layout>
    ...
    <tr class="border-b hover:bg-gray-50">
        <td class="py-3 px-2">{{ $donor->donor_name }}</td>
        ...
        <td class="py-3 px-2">
            <button type="button" 
                    onclick="alert('Contact: {{ $donor->donor_name }}\nPhone: {{ $donor->phone ?? 'N/A' }}\nEmail: {{ $donor->email ?? 'N/A' }}')" 
                    class="bg-indigo-600 text-white px-4 py-1 rounded text-sm font-bold hover:bg-indigo-700 shadow-sm">
                Contact
            </button>
        </td>
    </tr>
